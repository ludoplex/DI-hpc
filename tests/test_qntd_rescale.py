...
def qntd_rescale_val():
    ...
    hpc_loss, _ = hpc_qntd_rescale(hpc_q, hpc_next_n_q, hpc_action, hpc_next_n_action, hpc_reward, hpc_done, hpc_weight, gamma)
...
def qntd_rescale_perf():
    ...
    for i in range(times):
        ...
        if use_cuda:
            torch.cuda.synchronize()
        print(f'epoch: {i}, original qntd rescale cost time: {time.time() - t}')
    ...
    for i in range(times):
        ...
        if use_cuda:
            torch.cuda.synchronize()
        print(f'epoch: {i}, hpc qntd rescale cost time: {time.time() - t}')
