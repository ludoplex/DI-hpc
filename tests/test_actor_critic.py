...
def actor_critic_update_ae_val():
    ...
    print(f"actor critic update ae mean_relative_error: {str(mre)}")
...
def actor_critic_update_ae_perf():
    ...
    if use_cuda:
        torch.cuda.synchronize()
    print(f'original update ae cost time: {time.time() - t}')
    ...
    if use_cuda:
        torch.cuda.synchronize()
    print(f'hpc update ae cost time: {time.time() - t}')
...
def actor_critic_lstm_activation_val():
    ...
    mre = mean_relative_error(torch.flatten(ori_out).cpu().detach().numpy(), torch.flatten(hpc_out).cpu().detach().numpy())
    print(f"actor critic lstm activation mean_relative_error: {str(mre)}")
...
def actor_critic_lstm_activation_perf():
    ...
    if use_cuda:
        torch.cuda.synchronize()
    print(f'original lstm activation cost time: {time.time() - t}')
    ...
    if use_cuda:
        torch.cuda.synchronize()
    print(f'hpc lstm activation cost time: {time.time() - t}')
...
def actor_critic_pre_sample_val():
    ...
    mre = mean_relative_error(torch.flatten(ori_out).cpu().detach().numpy(), torch.flatten(hpc_out).cpu().detach().numpy())
    print(f"actor critic pre sample mean_relative_error: {str(mre)}")
...
def actor_critic_pre_sample_perf():
    ...
    if use_cuda:
        torch.cuda.synchronize()
    print(f'original pre sample cost time: {time.time() - t}')
    ...
    if use_cuda:
        torch.cuda.synchronize()
    print(f'hpc pre sample cost time: {time.time() - t}')


if __name__ == '__main__':
    print(
        f"target problem: batch_size = {batch_size}, max_entity_num = {max_entity_num}, input_dim = {input_dim}"
    )
    ...
    print(
        f"target problem: lstm_batch_size = {lstm_batch_size}, lstm_seq_len = {lstm_seq_len}, lstm_input_size = {lstm_input_size}, lstm_hidden_size = {lstm_hidden_size}, lstm_num_layers = {lstm_num_layers}"
    )
    ...
    print(
        f"target problem: lstm_batch_size = {lstm_batch_size}, lstm_seq_len = {lstm_seq_len}, lstm_hidden_size = {lstm_hidden_size}, max_entity_num = {max_entity_num}"
    )
